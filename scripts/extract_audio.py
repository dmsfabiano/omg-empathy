...
def extractAudio(path, savePath):
    ...
    for video in videos:
        ...
        videoNoExtension = os.path.splitext(video)[0]
        ...
        print ("--- Extracting audio:", savePath + "/" + videoNoExtension + ".wav" + " ...")

		# original
        command1 = 'ffmpeg -v quiet -i "{0}" -ac 1 -vn "{1}"'.format(videoPath, savePath + "/" + videoNoExtension + ".wav")
		# downsampled
        #command1 = 'ffmpeg -v quiet -i {0} -ar 125000 -ac 1 -vn {1}'.format(videoPath, savePath + "/" + videoNoExtension + ".wav")
