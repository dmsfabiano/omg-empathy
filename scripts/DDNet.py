...
def fusion(data_container):
    ...
    variance_container = [np.var(signal) for signal in data_container]
    variance_container = scaler.fit_transform(variance_container)
    
    weighted_container = np.asarray([0.0 for i in range(0,len(data_container[0]))])
...
def trainRegressor(model,x,y,epochs,batches,verb):
    model.fit(x, y, batch_size = epochs, epochs = batches, verbose=verb)
    return model
...
train_landmarks_subject = x_train[:,0:136].values
train_landmarks_actor = x_train[:,136:272].values
train_audio = x_train[:,272:5272].values

validation_landmarks_subject = x_validation[:,0:136].values
validation_landmarks_actor = x_validation[:,136:272].values
validation_audio = x_validation[:,272:5272].values
...
for i in range(0,len(train_landmarks_subject)):
    train_fused_faces.append([train_landmarks_subject[i],train_landmarks_actor[i]])
train_fused_faces = np.asarray(train_fused_faces)

for i in range(0,len(validation_landmarks_subject)):
    validation_fused_faces.append([validation_landmarks_subject[i],validation_landmarks_actor[i]])
...
faceFeatureExtractor = CreateRegressor(input_neurons = len(train_fused_faces[0]), output_neurons=100,hidden_layers=2,learning_rate=0.001, optimizer='adam',hidden_neurons=int((68+1)/2))
...
for instance in train_fused_faces:
    train_faceFeatures.append(getDeepFeatures(faceFeatureExtractor,instance))
train_faceFeatures = np.asarray(train_faceFeatures)

for instance in train_audio:
    train_audioFeatures.append(getDeepFeatures(audioFeatureExtractor,instance))
train_audioFeatures = np.asarray(train_audioFeatures)

# 4.2 validation
for instance in validation_fused_faces:
    validation_faceFeatures.append(getDeepFeatures(faceFeatureExtractor,instance))
validation_faceFeatures = np.asarray(validation_faceFeatures)

for instance in validation_audio:
    validation_audioFeatures.append(getDeepFeatures(audioFeatureExtractor,instance))
...
for i in range(0,len(train_faceFeatures)):
    train_fused_deep_features.append([train_faceFeatures[i],train_audioFeatures[i]])
train_fused_deep_features = np.asarray(train_fused_deep_features)

# 5.2 validation
for i in range(0,len(validation_faceFeatures)):
    validation_fused_deep_features.append([validation_faceFeatures[i],validation_audioFeatures[i]])
